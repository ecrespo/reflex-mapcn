...
### T-020 · Revisión E2E manual — `[x] 2026-09-12`
...
- **Checklist (2026-09-12, ejecutado con Playwright sobre `reflex run` y claves reales):**
  ...
  | Tamaño del histórico (RNF ≤ 400 KB) | Falló con 1 089 KB (H-03); tras el Delta `2026-09-catalog-payload`, **335,4 KB** medidos en el mismo sitio |
...
- **Hallazgos de la revisión:**
  ...
  - **H-02 (2026-09-12, CERRADO):** `uv run reflex run` sincroniza el entorno sin el extra `dev` y deja el `.venv` sin pytest ni ruff. Documentado `uv run --extra dev reflex run` en el README de la demo y en el del paquete, con un test que lo exige (`test_the_readme_runs_the_demo_without_pruning_the_dev_tools`). Efecto lateral conocido: un `uv run` lanzado dentro de `mapcn_demo/` puede dejar ahí un `.venv` vacío, y ese directorio hace que `reflex component build` se salga del repositorio al recorrer el árbol; borrarlo es seguro.
...
- **Notas de revisión (2026-09-12):**
  ...
  - **Galería de Reflex: bloqueado por el lado de Reflex (2026-09-12).** `reflex component share` autentica y acepta el nombre del paquete, pero el registro falla con `405 Method Not Allowed` contra `https://gallery-backend.reflex.dev/custom-components/gallery`. Comprobado que no es cosa nuestra: el mismo endpoint responde 200 a GET y 405 a POST y a PUT, el CLI de hosting 0.1.71 y 0.1.72 se comportan igual, y el error no es de autenticación (sería 401 o 403). La galería tiene hoy 34 entradas y `reflex-mapcn` no está. La documentación de Reflex sigue anunciando el comando, así que el endpoint está retirado o roto sin actualizar los docs. Queda para reintentar más adelante o para reportarlo a Reflex; no bloquea la release, que ya está en PyPI y en GitHub.
...
|---|---|---|---|
| 2026-09-12 | H-02 | OK | Test primero, en rojo, y luego la documentación en los dos README. Se explica el porqué y no solo el comando: `uv run` sincroniza antes de ejecutar, así que arrancar la demo sin el extra desinstala pytest, ruff y twine. Con esto la revisión T-020 no deja hallazgos abiertos. |
