...
| A-09 | MEDIO | Consistencia | `LayerFeatureEvent` (0.2) y `MapGeoJSONEvent` (0.1.0) tienen la misma forma con nombres distintos. | API §2.3, `mapcn.py` | En T-013 exportar `LayerFeatureEvent` como alias de `MapGeoJSONEvent` (mismo TypedDict) y documentar | Pendiente (recomendado) |
